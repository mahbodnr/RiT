...
MixtureOfExpertsReturn = namedtuple(
    ...
    ["outputs", "total_aux_loss", "balance_loss", "router_z_loss"],
)
...
class Expert(Module):
    def __init__(self, dim, hidden_mult=4, mult_bias=True, prenorm=False):
        super().__init__()
...
class MoE(Module):
    ...
    def __init__(
        ...
        experts = default(
            experts,
            lambda: [
                Expert(dim=dim, hidden_mult=expert_hidden_mult)
                for _ in range(num_experts)
            ],
        )

        self.experts = Experts(
            experts, is_distributed=is_distributed, allow_var_seq_len=allow_var_seq_len
        )
    ...
    def forward(self, x, noise_gates=False, noise_mult=1.0):
        dispatch_tensor, combine_tensor, balance_loss, router_z_loss = self.gate(
            x, noise_gates=noise_gates, noise_mult=noise_mult
        )
...
class SparseMoEBlock(Module):
    ...
    @beartype
    def __init__(self, moe: MoE, *, add_ff_before=False, add_ff_after=True):
        super().__init__()
        dim = moe.dim
    ...
    def forward(self, x, noise_gates=False, noise_mult=1.0):
        ...
        moe_out, total_aux_loss, balance_loss, router_z_loss = self.moe(
            self.moe_prenorm(x), noise_gates=noise_gates, noise_mult=noise_mult
        )
